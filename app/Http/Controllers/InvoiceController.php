...
class InvoiceController extends Controller
{
    ...
    public function sendZatcaInvoice(Invoice $invoice) {
        ...
        if ($invoice->zatca_status == 'sent without error') {
            ...
            return redirect()->back()->with('error', 'فشل إرسال الفاتورة إلى هيئة الزكاة والدخل, ' . '<a class="text-white fw-bold" href="'.route('invoices.zatca.invoice', $invoice).'">عرض رد ZATCA</a>');
    ...
    public function zatcaInvoice(Invoice $invoice) {
        $zatcaInvoice = $invoice->zatcaInvoice;
        $responseLog = null;
        try {
            $responseLog = is_string($zatcaInvoice->response_log)
                ? json_decode($zatcaInvoice->response_log, true)
                : $zatcaInvoice->response_log;
        } catch (\Exception $e) {
            $responseLog = null;
        }
        return view('pages.invoices.zatca_invoice_details', compact('zatcaInvoice', 'invoice', 'responseLog'));
    }
}
