...
class CompanyController extends Controller
{
    ...
    public function updateCompany(CompanyRequest $request, Company $company) {
        ...
        $new = $company->toArray();
        logActivity('تحديث بيانات الشركة', "تم تحديث بيانات الشركة " . $company->name, $old, $new);
    ...
    public function storeAddress(CompanyAddressRequest $request, Company $company) {
        ...
        $new = $company->address->toArray();
        logActivity('إضافة بيانات العنوان الوطني', "تم إضافة بيانات العنوان الوطني للشركة " . $company->name, null, $new);
    ...
    public function updateAddress(CompanyAddressRequest $request, Company $company) {
        ...
        $old = $company->address->toArray();
        $validated = $request->validated();
        $company->address()->update($validated);
        $company->refresh();
        $new = $company->address->toArray();
        logActivity('تحديث بيانات العنوان الوطني', "تم تحديث بيانات العنوان الوطني للشركة " . $company->name, $old, $new);
    ...
    public function storeBankNumber(Request $request, Company $company) {
        ...
        $new = $company->bankAccounts()->latest()->first()->toArray();
        logActivity('إضافة رقم بنك', "تم إضافة رقم بنك جديد للشركة " . $company->name, null, $new);
    ...
    public function updateBankNumber(Request $request, $bankAccountId) {
        ...
        $old = BankAccount::findOrFail($bankAccountId)->toArray();
        ...
        logActivity('تحديث رقم بنك', "تم تحديث رقم بنك للشركة " . $bankAccount->company->name, $old, $new);
    ...
    public function deleteBankNumber($bankAccountId) {
        ...
        logActivity('حذف رقم بنك', "تم حذف رقم بنك للشركة " . $bankAccount->company->name, $old, null);
