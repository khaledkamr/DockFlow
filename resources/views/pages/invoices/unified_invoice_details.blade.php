...
@section('content')
    ...
    @if(!$invoice->is_posted && $invoice->type != 'تخليص')
        <div class="modal fade" id="postInvoiceModal" tabindex="-1" aria-labelledby="postInvoiceModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header bg-primary">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    @if($invoice->type == 'تخزين و شحن')
                                        <tr>
                                            ...
                                            <td class="text-center text-success fw-bold">
                                                {{ number_format($invoice->storage_amount, 2) }}
                                            </td>
                                            <td class="text-center">
                                                ايرادات تخزين فاتورة رقم {{ $invoice->code }}
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="text-center">
                                            ...
                                            <td class="text-center">
                                                ايرادات شحن فاتورة رقم {{ $invoice->code }}
                                            </td>
                                        </tr>
                                    @else
                                        <tr>
                                            <td class="text-center">
                                                {{ $invoice->type == 'خدمات' ? 'ايرادات متنوعة' : 'ايرادات ' . $invoice->type }}
                                            </td>
                                            <td class="text-center">0.00</td>
                                            <td class="text-center text-success fw-bold">
                                            ...
                                            <td class="text-center">
                                                ايرادات
                                                {{ $invoice->type == 'تخزين' || $invoice->type == 'شحن' ? $invoice->type : 'متنوعة' }}
                                    ...
                                    <tr>
                                        <td class="text-center">
                                            {{ $invoice->customer->account->name ?? $invoice->customer->name }}
                                        </td>
                                        ...
                                        <td class="text-center">0.00</td>
                                        <td class="text-center">
                                            استحقاق فاتورة {{ $invoice->type }} رقم {{ $invoice->code }}
                                        </td>
                                    </tr>
                                </tbody>
    ...
    @elseif(!$invoice->is_posted && $invoice->type == 'تخليص')
        @php
                        ...
                        <table class="table table-striped table-hover">
                            <thead class="table-primary">
                                <tr>
                                    ...
                                    <th class="text-center bg-dark text-white text-nowrap">سعر التخزين</th>
                                    <th class="text-center bg-dark text-white text-nowrap">أيام التأخير</th>
                                    <th class="text-center bg-dark text-white text-nowrap">غرامة التأخير</th>
                                    <th class="text-center bg-dark text-white text-nowrap">خدمات</th>
                            ...
                            <tbody>
                                @foreach ($invoice->containers as $container)
                                    <tr>
                                        ...
                                        <td class="text-center fw-bold">{{ $container->storage_days }} يوم</td>
                                        ...
                                        <td class="text-center fw-bold">{{ number_format($container->total_services, 2) }}
                                        </td>
                                        <td class="text-center fw-bold">{{ number_format($container->total, 2) }}
                                ...
                                @foreach ($invoice->shippingPolicies as $index => $policy)
                                    <tr>
                                        ...
                                        <td class="text-center fw-bold text-nowrap">
                                            {{ $policy->goods->first()->description ?? '---' }}
                                        </td>
                            ...
                            <tbody>
                                @foreach ($invoice->containers as $container)
                                    <tr>
                                        ...
                                        <td class="text-center fw-bold">{{ $container->services->first()->pivot->price }} <i
                            ...
                            <tbody>
                                @foreach ($invoice->containers as $container)
                                    <tr>
                                        <td class="text-center fw-bold">{{ $loop->iteration }}</td>
                                        <td class="text-center fw-bold">
                                            <a href="{{ route('transactions.details', $container->transactions->first()) }}"
                                                class="text-decoration-none">
