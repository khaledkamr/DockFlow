                    ...
                    <div class="row mb-3">
                        ...
                        <script>
                                ...
                                $.ajax({
                                    ...
                                    error: function(xhr, status, error) {
                                        ...
                                        $('#invoicesTableBody').html(
                                            '<tr><td colspan="11" class="text-center text-danger">حدث خطأ أثناء تحميل الفواتير</td></tr>'
                                        );
                                    }
                                });
                            });
                            ...
                            function displayInvoices(invoices) {
                                ...
                                invoices.forEach((invoice, index) => {
                                    ...
                                    if (invoice.status == 'تم الدفع') {
                                        paymentStatusBadge = '<span class="badge bg-success">مدفوع</span>';
                                    } else if (invoice.status == 'تم الدفع جزئياً') {
                                        paymentStatusBadge = '<span class="badge bg-warning">مدفوع جزئياً</span>';
                                    } else {
                                        paymentStatusBadge = '<span class="badge bg-danger">غير مدفوع</span>'
                                    }

                                    const apiPaidAmount = parseFloat(invoice.paid_amount || 0);
                                    const totalAmount = parseFloat(invoice.total_amount || 0);
                                    const remainingBalance = totalAmount - apiPaidAmount;

                                    html += `
                                        <tr class="invoice-row" data-invoice-id="${invoice.id}" data-invoice-code="${invoice.code}" data-total-amount="${totalAmount}" data-api-paid-amount="${apiPaidAmount}">
                                            <td class="text-center">
                                                <input type="checkbox" class="form-check-input invoice-checkbox" 
                                                    ...
                                                    data-api-paid-amount="${apiPaidAmount}">
                                            </td>
                                            ...
                                            <td class="text-center fw-bold">${parseFloat(remainingBalance || 0).toFixed(2)}</td>
                                            <td class="text-center"><input type="number" step="any" min="0" max="${remainingBalance.toFixed(2)}" name="paid_amount" class="form-control form-control-sm paid-amount-input" data-invoice-id="${invoice.id}" data-max-amount="${remainingBalance.toFixed(2)}"></td>
                                            <td class="text-center balance-amount">${remainingBalance.toFixed(2)}</td>
                                ...
                                // Add event listeners to paid amount inputs
                                $('.paid-amount-input').on('input', function() {
                                    ...
                                    const totalAmount = parseFloat($row.data('total-amount'));
                                    const apiPaidAmount = parseFloat($row.data('api-paid-amount'));
                                    const remainingBalance = totalAmount - apiPaidAmount;
                                    ...
                                    // Update balance: (total_amount - api_paid_amount) - user_paid_amount
                                    const userPaidAmount = Math.min(parseFloat($(this).val()) || 0, maxAmount);
                                    const newBalance = remainingBalance - userPaidAmount;
                                    $row.find('.balance-amount').text(newBalance.toFixed(2));

                                    // Update selected invoices if checkbox is checked
                                    if ($row.find('.invoice-checkbox').is(':checked')) {
                                        updateSelectedInvoiceAmount(invoiceId);
                                        updateSelectedSummary();
                                    }
                                });
                                ...
                                $('.invoice-checkbox').on('change', function() {
                                    ...
                                    const apiPaidAmount = parseFloat($(this).data('api-paid-amount'));
                                    ...
                                    if ($(this).is(':checked')) {
                                        ...
                                        selectedInvoices.push({
                                            ...
                                            apiPaidAmount: apiPaidAmount,
                                            userPaidAmount: 0 // Initial paid amount is 0
                                        });
                            ...
                            function updateSelectedSummary() {
                                ...
                                const total = selectedInvoices.reduce((sum, inv) => sum + (inv.userPaidAmount || 0), 0);
                            ...
                            function updateSelectedInvoiceAmount(invoiceId) {
                                const $row = $(`.invoice-row[data-invoice-id="${invoiceId}"]`);
                                const userPaidAmount = parseFloat($row.find('.paid-amount-input').val()) || 0;

                                // Update the selectedInvoices array with the new paid amount
                                const invoice = selectedInvoices.find(inv => inv.id == invoiceId);
                                if (invoice) {
                                    invoice.userPaidAmount = userPaidAmount;
                                }
                            }
                            ...
                            $('#submitSelectedInvoices').on('click', function() {
                                ...
                                const totalAmount = selectedInvoices.reduce((sum, inv) => sum + (inv.userPaidAmount || 0), 0);

                                // Get all invoice codes
                                const invoiceCodes = selectedInvoices.map(inv => inv.code).join(' - ');

                                // Clear and populate payments collection
                                $('#invoice_ids_container').empty();
                                selectedInvoices.forEach(inv => {
                                    $('#invoice_ids_container').append(
                                        `<input type="hidden" name="payments[${inv.id}][invoice_id]" value="${inv.id}">` +
                                        `<input type="hidden" name="payments[${inv.id}][amount]" value="${(inv.userPaidAmount || 0).toFixed(2)}">`
                                    );
                                });
