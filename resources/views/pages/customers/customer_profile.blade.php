                ...
                <div class="row g-3 mb-4">
                    <div class="col-6 col-md-4">
                        <div class="text-center p-3 border rounded">
                            ...
                            <div class="fw-bold">
                                {{ Carbon\Carbon::parse($customer->contract->start_date)->format('Y/m/d') }}</div>
                        </div>
                    </div>
                    <div class="col-6 col-md-4">
                        <div class="text-center p-3 border rounded">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @elseif($container->status == 'تم التسليم')
                                        <div class="status-delivered">{{ $container->status }} <i
                                ...
                                <td class="{{ $container->location ? 'fw-bold' : 'text-muted' }}">
                                    {{ $container->location ?? 'غير محدد' }}</td>
                                <td>{{ $container->date ? Carbon\Carbon::parse($container->date)->format('Y/m/d') : '-' }}
                                </td>
                                <td>{{ $container->exit_date ? Carbon\Carbon::parse($container->exit_date)->format('Y/m/d') : '-' }}
                                </td>
        ...
        <div
            ...
            <div class="d-flex gap-2 flex-wrap">
                <span class="badge bg-success">
                    مسددة: {{ collect($customer->invoices)->where('status', 'تم الدفع')->count() }}
                </span>
                <span class="badge bg-warning">
                    مسددة جزئياً: {{ collect($customer->invoices)->where('status', 'تم الدفع جزئياً')->count() }}
                </span>
                <span class="badge bg-danger">
                    غير مسددة: {{ collect($customer->invoices)->where('status', 'لم يتم الدفع')->count() }}
                </span>
            </div>
        </div>
        <div class="card-body p-0">
                ...
                <div class="d-flex flex-column flex-md-row justify-content-center gap-3">
                    <div class="card text-center px-3 px-md-5 py-2 border-success flex-grow-1 flex-md-grow-0">
                        ...
                        <div class="fw-bold text-success">
                            {{ collect($customer->invoices)->sum('paid_amount') }}
                    ...
                    <div class="card text-center px-3 px-md-5 py-2 border-danger flex-grow-1 flex-md-grow-0">
                        ...
                        <div class="fw-bold text-danger">
                            {{ collect($customer->invoices)->sum('total_amount') - collect($customer->invoices)->sum('paid_amount') }}
                            <i data-lucide="saudi-riyal"></i>
                        </div>
                    </div>
